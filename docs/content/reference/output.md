...
describes what users get. Pick a format with `-o`, or let echr choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
echr <command> -o table   # aligned columns for reading
echr <command> -o jsonl   # one JSON object per line, for piping
echr <command> -o json    # a single JSON array
echr <command> -o csv     # spreadsheet friendly
echr <command> -o tsv     # tab-separated
echr <command> -o url     # just the URL column
echr <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
echr <command> --fields id,title,url
```
...
```bash
echr <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
echr <command>            # a table, because this is a terminal
echr <command> | wc -l    # JSONL, because this is a pipe
```
